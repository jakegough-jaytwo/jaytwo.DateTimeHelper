...
DOCKER_TAG?=jaytwo_datetimehelper
...
build: restore
	dotnet build ./jaytwo.DateTimeHelper.sln

test: unit-test
  
unit-test: build
	rm -rf out/testResults
	dotnet test ./test/jaytwo.DateTimeHelper.Tests \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.DateTimeHelper.Tests.trx"

pack:
	rm -rf out/packed
	cd ./src/jaytwo.DateTimeHelper; \
